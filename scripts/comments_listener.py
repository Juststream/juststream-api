...
def reply_comment(comment):
    ...
    job_id = media_convert_client.create_streaming_videos(
        [f"s3://m3u8-files-mate/{filepath}"], video_id
    )
    ...
    videos_table.put_item(item_to_put)
...
if __name__ == "__main__":
    ...
    while True:
        try:
            for comment in reddit_client.reddit.subreddit(subreddit).stream.comments(
                skip_existing=True
            ):
