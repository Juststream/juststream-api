...
class DynamoClient:
    ...
    def __init__(self, table_name: str, primary_key='id', secondary_key=None):
        ...
        self.table = boto3.resource('dynamodb', region_name='us-east-1').Table(self.table_name)
    ...
    def batch_write(self, items: list, override=False):
        ...
        """
        if override:
            batch_writer = self.table.batch_writer(overwrite_by_pkeys=[self.primary_key])
        else:
    ...
    def update_item(self, _id: str, args: List[tuple], return_values: str = 'ALL_NEW') -> dict:
        """
        ...
        """
        query = ', '.join([f'#{x[0]}=:{x[0]}' for x in args])  # Creating structure Dynamo DB asks for
