...
def get_players_from_team(team):
    ...
    for player in team["Ps"]:
        if (
            player.get("Pon") == "COACH"
            or not player.get("Snm")
            or not player.get("Snu")
        ):
            continue
        players.append({"name": player["Snm"], "shirt_number": player["Snu"]})
    return players
...
def get_match_details(match_id: str):
    ...
    current_time = (
        int(current_time_raw.rstrip("'")) if current_time_raw[0].isdigit() else None
    )
