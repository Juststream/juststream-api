...
def handler(event, context):
    ...
    if isinstance(header_overrides, dict):
        header_overrides = ";".join(
            [f"{key}={value}" for key, value in header_overrides.items()]
        )
    if header_overrides:
    ...
    try:
        ...
        data += fd.read(bytes_size)
        data += fd.read(bytes_size)
        s3_client.put_object(Body=data, Bucket="m3u8-files-mate", Key=output_path)
    finally:
        fd.close()
    return {"headers": {}, "isBase64Encoded": True, "statusCode": 200, "body": ""}
